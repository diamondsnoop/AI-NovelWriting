...
class SmokeTest(unittest.TestCase):
    def test_init_write_resume_smoke(self) -> None:
        ...
        try:
            ...
            volume_plan_path = Path(plan_result["volume_plan_path"])
            beat_sheet_path = Path(plan_result["beat_sheet_path"])
            timeline_path = Path(plan_result["timeline_path"])
            outline_text = outline_path.read_text(encoding="utf-8")
            compact_outline_text = compact_outline_path.read_text(encoding="utf-8")
            volume_plan_text = volume_plan_path.read_text(encoding="utf-8")
            beat_sheet_text = beat_sheet_path.read_text(encoding="utf-8")
            ...
            self.assertIn("Volume 1 Plan", volume_plan_text)
            self.assertIn("Beat Sheet", beat_sheet_text)
            ...
            self.assertIn("Volume plan", chapter_text)
            self.assertIn("Chapter beats", chapter_text)
            self.assertIn("Chapter timeline", chapter_text)
